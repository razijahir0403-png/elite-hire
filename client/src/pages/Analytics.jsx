...
const Analytics = () => {
  ...
  const handleEditClick = (record) => {
    ...
    setFormData({
      ...
      salaryPackage: record.salaryPackage || '',
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-extrabold tracking-wider text-slate-500">
                  ...
                  <th onClick={() => handleSort('salaryPackage')} className="px-6 py-4 cursor-pointer hover:text-slate-800">
                    <div className="flex items-center space-x-1">
                      <span>Compensation</span>
                      {sortBy === 'salaryPackage' && (sortOrder === 'asc' ? <ChevronUp size={12} /> : <ChevronDown size={12} />)}
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white text-xs">
                {records.map((r) => (
                  <tr key={r._id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-700 font-medium">{r.salaryPackage}</td>
                    ...
                    <td className="px-6 py-4 text-slate-600">
                      ...
                      <div className="text-[10px] text-slate-450">{r.contactNumber}</div>
                    </td>
      ...
      >
        <form onSubmit={handleAddEditSubmit} className="space-y-4 text-slate-800">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1.5">Salary Package *</label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1.5">Contact Number *</label>
              <input
                type="text"
                required
