...
export const PAYMENT_STATUS = { PENDING: 0, PARTIALLY_PAID: 1, PAID: 2 };
...
export const PaymentStatus = { 0: 'Pending', 1: 'Partially Paid', 2: 'Paid' };
...
export const StatusColors = {
  payment: {
    ...
    1: 'bg-yellow-50 text-yellow-800 border-yellow-200',
...
export const getStatusLabel = (type, code) => {
  ...
  const map = maps[type] || PaymentStatus;
...
export const getStatusBadgeClass = (type, code) => {
  const key = Number(code);
  const typeMap = StatusColors[type] || StatusColors.payment;
  return typeMap[key] || 'bg-slate-50 text-slate-600 border-slate-200';
};

export const paymentFilterOptions = Object.entries(PaymentStatus).map(([value, label]) => ({
  label,
  value: Number(value),
}));
